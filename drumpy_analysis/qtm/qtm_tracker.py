...
import qtm_rt
from drumpy.drum.drum import Drum, DrumPresets, SleepOption
from drumpy.drum.foot import Foot
from drumpy.drum.hand import Hand
...
class QTMFullTracker:
    def __init__(self: Self, drum: Drum, log_to_file: bool = False) -> None:
        self.left_hand = Hand(
        ...
        if log_to_file:
            ...
            self.csv_writer = TrajectoryFile(log_file)
    ...
    async def start_capture(self: Self) -> None:
...
async def main() -> None:
    drum = Drum(
        200,
        40,
        presets=DrumPresets.first_qtm_recording(),
        sleep_option=SleepOption.NO_SLEEP,
    )
